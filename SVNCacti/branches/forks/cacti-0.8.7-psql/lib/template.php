		...
		foreach ($input_fields as $input_field) {
			...
			if (!eregi('^' . VALID_HOST_FIELDS . '$', $input_field["type_code"])) {
				...
				if ($input_field["t_value"] == "") { /* template this value */
					db_replace("data_input_data",array("data_input_field_id"=>$input_field["id"],"data_template_data_id"=>$data_source["id"],"value"=>$input_field["value"]),array("data_input_field_id","data_template_data_id"),true);
				}
				...
				*/
//				error_log("in lib/template.php push_out_data_source_custom_data db_replace 2");
				db_replace("data_input_data",array("data_input_field_id"=>$input_field["id"],"data_template_data_id"=>$data_source["id"],"value"=>$input_field["value"]),array("data_input_field_id","data_template_data_id"),true);
...
function change_data_template($local_data_id, $data_template_id) {
	...
	$save["id"] = ($data["id"] === NULL) ? 0 : $data["id"];
	...
	foreach ($data_input_data as $item) {
		...
		if (($new_save == true) || (empty($item["t_value"]))) {
			db_replace("data_input_data",array("data_input_field_id"=>$item["data_input_field_id"],"data_template_data_id"=>$data_template_data_id,"t_value"=>$item["t_value"],"value"=>$item["value"]),array("data_input_field_id","data_template_data_id"),true);
		}
	}
	}
...
function push_out_graph_item($graph_template_item_id) {
	...
	$graph_item_inputs = db_fetch_assoc("select
		...
		from (graph_template_input cross join graph_template_input_defs)
...
function change_graph_template($local_graph_id, $graph_template_id, $intrusive) {
	...
	$save["id"] = ($graph_list["id"] === NULL) ? 0 : $graph_list["id"];
	...
	sql_save($save, "graph_templates_graph", "id");
...
function graph_to_graph_template($local_graph_id, $graph_title) {
	...
	/* FIXME: can use last_insert_id */
	$graph_template_id = db_fetch_cell("select max(id)+1 from graph_templates");
	db_execute("insert into graph_templates (id,name,md5hash) values ($graph_template_id,'" . str_replace("<graph_title>", db_fetch_cell("select title from graph_templates_graph where local_graph_id=$local_graph_id"), $graph_title) . "','" . get_md5hash_graph_template(0) . "')");
...
function data_source_to_data_template($local_data_id, $data_source_title) {
	/* create a new graph template entry */
	/* FIXME: can use last_insert_id */
	$data_template_id = db_fetch_cell("select max(id)+1 from data_template");
	db_execute("insert into data_template (id,name,md5hash) values ($data_template_id,'" . str_replace("<ds_title>", db_fetch_cell("select name from data_template_data where local_data_id=$local_data_id"), $data_source_title) . "','" .  get_md5hash_data_template(0) . "')");
