...
function &xml_to_data_template($hash, &$xml_array, &$hash_cache, $import_custom_rra_settings) {
	...
	unset($save);
	$save["id"] = (empty($_data_template_id) ? "0" : db_fetch_cell("select data_template_data.id from (data_template cross join data_template_data) where data_template.id=data_template_data.data_template_id and data_template.id=$data_template_id and data_template_data.local_data_id=0"));
		...
		if (!empty($hash_items[0])) {
			for ($i=0; $i<count($hash_items); $i++) {
				...
				if (isset($hash_cache["round_robin_archive"]{$parsed_hash["hash"]})) {
					/* db_replace() doesn't replace primary key, delete then insert */
					db_execute("delete from data_template_data_rra where data_template_data_id = $data_template_data_id and rra_id = ".$hash_cache["round_robin_archive"]{$parsed_hash["hash"]});
					db_execute("insert into data_template_data_rra (data_template_data_id,rra_id) values ($data_template_data_id,".$hash_cache["round_robin_archive"]{$parsed_hash["hash"]} . ")");
				}
			}
		}
...
function &xml_to_round_robin_archive($hash, &$xml_array, &$hash_cache) {
	...
	if (!empty($hash_items[0])) {
		for ($i=0; $i<count($hash_items); $i++) {
			/* db_replace() doesn't replace primary key, delete then insert */
			db_execute("delete from rra_cf where rra_id = $rra_id and consolidation_function_id = ".$hash_items[$i]);
			db_execute("insert into rra_cf (rra_id,consolidation_function_id) values ($rra_id," . $hash_items[$i] . ")");
		}
	}
	...
	if (!empty($hash_items[0])) {
		for ($i=0; $i<count($hash_items); $i++) {
			...
			if (isset($hash_cache["graph_template"]{$parsed_hash["hash"]})) {
				/* db_replace() doesn't replace primary key, delete then insert */
				db_execute("delete from host_template_graph where host_template_id = $host_template_id and graph_template_id = ".$hash_cache["graph_template"]{$parsed_hash["hash"]});
				db_execute("insert into host_template_graph (host_template_id,graph_template_id) values ($host_template_id,".$hash_cache["graph_template"]{$parsed_hash["hash"]}.")");
			}
		}
	}
	...
	if (!empty($hash_items[0])) {
		for ($i=0; $i<count($hash_items); $i++) {
			...
			if (isset($hash_cache["data_query"]{$parsed_hash["hash"]})) {
				/* db_replace() doesn't replace primary key, delete then insert */
				db_execute("delete from host_template_snmp_query where host_template_id=$host_template_id and snmp_query_id=".$hash_cache["data_query"]{$parsed_hash["hash"]});
				db_execute("insert into host_template_snmp_query (host_template_id,snmp_query_id) values ($host_template_id,".$hash_cache["data_query"]{$parsed_hash["hash"]}.")");
			}
		}
	}
