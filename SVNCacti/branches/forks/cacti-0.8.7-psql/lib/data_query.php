		...
		if ($field_array["direction"] == "input") {
			...
			for ($i=0;($i<sizeof($script_data_array));$i++) {
				if (preg_match("/(.*)" . preg_quote($script_queries["output_delimeter"]) . "(.*)/", $script_data_array[$i], $matches)) {
					...
					db_replace("host_snmp_cache",array("host_id"=>$host_id,"snmp_query_id"=>$snmp_query_id,"field_name"=>$field_name,"field_value"=>$field_value,"snmp_index"=>$script_index,"objid"=>""),array("host_id","snmp_query_id", "field_name", "snmp_index"),true);
	...
	while (list($field_name, $field_array) = each($snmp_queries["fields"])) {
		if ((!isset($field_array["oid"])) && ($field_array["source"] == "index")) {
			for ($i=0; $i<sizeof($snmp_index); $i++) {
				...
				db_replace("host_snmp_cache",array("host_id"=>$host_id,"snmp_query_id"=>$snmp_query_id,"field_name"=>$field_name,"field_value"=>$snmp_index[$i]["value"],"snmp_index"=>$snmp_index[$i]["value"],"objid"=>""),array("host_id","snmp_query_id", "field_name", "snmp_index"),true);
			}
			...
			if ($field_array["source"] == "value") {
				for ($i=0; $i<sizeof($snmp_index); $i++) {
					...
					db_replace("host_snmp_cache",array("host_id"=>$host_id,"snmp_query_id"=>$snmp_query_id,"field_name"=>$field_name,"field_value"=>$value,"snmp_index"=>$snmp_index[$i]["value"],"objid"=>$oid),array("host_id","snmp_query_id", "field_name", "snmp_index"),true);
				}
			}
			...
			if ($field_array["source"] == "value") {
				for ($i=0; $i<sizeof($snmp_data); $i++) {
					...
					db_replace("host_snmp_cache",array("host_id"=>$host_id,"snmp_query_id"=>$snmp_query_id,"field_name"=>$field_name,"field_value"=>$snmp_data[$i]["value"],"snmp_index"=>$snmp_index,"objid"=>$oid),array("host_id","snmp_query_id", "field_name", "snmp_index"),true);
				}
				...
				for ($i=0; $i<sizeof($snmp_data); $i++) {
					...
					db_replace("host_snmp_cache",array("host_id"=>$host_id,"snmp_query_id"=>$snmp_query_id,"field_name"=>$field_name,"field_value"=>$value,"snmp_index"=>$snmp_index,"objid"=>$oid),array("host_id","snmp_query_id", "field_name", "snmp_index"),true);
				}
...
function get_formatted_data_query_indexes($host_id, $data_query_id) {
	...
	$sort_field_data = array_rekey(db_fetch_assoc("select
		...
		group by graph_local.snmp_index,host_snmp_cache.field_value"), "snmp_index", "field_value");
