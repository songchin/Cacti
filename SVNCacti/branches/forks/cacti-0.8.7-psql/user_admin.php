...
function form_save() {
	...
	if ((isset($_POST["save_component_graph_perms"])) && (!is_error_message())) {
		...
		if (isset($_POST["add_graph_y"])) {
			/* This might not work as it looks like we replace the primary key */
			db_replace("user_auth_perms", array("user_id"=>get_request_var_post("id"), "item_id"=>get_request_var_post("perm_graphs"), "type"=>1), array("user_id", "item_id", "type"), true);
			$add_button_clicked = true;
		}elseif (isset($_POST["add_tree_y"])) {
			db_replace("user_auth_perms", array("user_id"=>get_request_var_post("id"), "item_id"=>get_request_var_post("perm_trees"), "type"=>2), array("user_id", "item_id", "type"), true);
			$add_button_clicked = true;
		}elseif (isset($_POST["add_host_y"])) {
			db_replace("user_auth_perms", array("user_id"=>get_request_var_post("id"), "item_id"=>get_request_var_post("perm_hosts"), "type"=>3), array("user_id", "item_id", "type"), true);
			$add_button_clicked = true;
		}elseif (isset($_POST["add_graph_template_y"])) {
			db_replace("user_auth_perms", array("user_id"=>get_request_var_post("id"), "item_id"=>get_request_var_post("perm_graph_templates"), "type"=>4), array("user_id", "item_id", "type"), true);
			$add_button_clicked = true;
		}
				...
				while (list($var, $val) = each($_POST)) {
					if (eregi("^[section]", $var)) {
						if (substr($var, 0, 7) == "section") {
							/* Check primary key, if necessary replace with DELETE/INSERT */
							db_replace("user_auth_realm", array("user_id"=>$user_id, "realm_id"=>substr($var, 7)), array("realm_id", "user_id"), true);
						}
					}
				}
					...
					while (list($field_name, $field_array) = each($tab_fields)) {
						if ((isset($field_array["items"])) && (is_array($field_array["items"]))) {
							while (list($sub_field_name, $sub_field_array) = each($field_array["items"])) {
								db_replace("settings_graphs",  array("user_id"=>(!empty($user_id) ? $user_id : get_request_var_post("id")),"name"=>"$sub_field_name","value"=>get_request_var_post($sub_field_name, "")), array("user_id","name"), true);
							}
						}else{
							db_replace("settings_graphs",  array("user_id"=>(!empty($user_id) ? $user_id : $_POST["id"]),"name"=>"'$field_name'","value"=>get_request_var_post($field_name)), array("user_id","name"), true);
						}
					}
				}
...
function graph_perms_edit() {
	...
	$hosts = db_fetch_assoc("SELECT
		host.id,
		".sql_function_concat("host.description","' ('","host.hostname","')'")." as name
		FROM host
	...
	<table align='center' width='100%'>
		<tr>
			<td nowrap>Add Host:&nbsp;
				<?php form_dropdown("perm_hosts",db_fetch_assoc("SELECT id,".sql_function_concat("description","' ('","hostname","')'")." AS name FROM host ORDER BY description,hostname"),"name","id","","","");?>
			</td>
			<td align="right">
...
function user() {
	...
	$user_list = db_fetch_assoc("SELECT
		id,
		max(user_auth.username) as username,
		max(full_name) as full_name,
		max(realm) as realm,
		max(enabled) as enabled,
		max(policy_graphs) as policy_graphs,
		max(time) as time,
		max(time) as dtime
		FROM user_auth
		...
		read_config_option("num_rows_device"),
		read_config_option("num_rows_device") * (get_request_var_request("page") - 1));
