	...
	if ($_POST["drp_action"] == "1") { /* delete */
		...
		if (isset($ds_array)) {
			$graphs = db_fetch_assoc("select
				...
				and graph_templates_graph.local_graph_id > 0
				group by graph_templates_graph.local_graph_id,graph_templates_graph.title_cache
		...
		print "	<tr>
				<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
					...
					<p>$ds_list</p>
					<p><strong>New Host:</strong><br>"; form_dropdown("host_id",db_fetch_assoc("select id,".sql_function_concat("host.description","' ('","host.hostname","')'")." as name from host order by description,hostname"),"name","id","","","0"); print "</p>
				</td>
			</tr>\n
			";
	}elseif ($_POST["drp_action"] == "4") { /* duplicate */
...
function ds_edit() {
	...
	$form_array = array(
		...
		"host_id" => array(
			...
			"none_value" => "None",
			"sql" => "select id,".sql_function_concat("host.description","' ('","host.hostname","')'")." as name from host order by description,hostname"
			),
...
function ds() {
	...
	$total_rows = sizeof(db_fetch_assoc("SELECT
		data_local.id
		FROM (data_local CROSS JOIN data_template_data)
	...
	$data_sources = db_fetch_assoc("SELECT
		...
		read_config_option("num_rows_data_source"), (read_config_option("num_rows_data_source")*($_REQUEST["page"]-1)));
