...
int main(int argc, char *argv[]) {
	...
	if (set.verbose == POLLER_VERBOSITY_DEBUG) {
		snprintf(logmessage, LOGSIZE-1, "CACTID: Version %s starting", VERSION);
	...
	if (set.verbose == POLLER_VERBOSITY_DEBUG) {
		snprintf(logmessage, LOGSIZE-1, "DEBUG: Parent pid=%i, Script Server pid=%i", set.cactid_pid, set.php_sspid);
		cacti_log(logmessage, SEV_DEBUG, 0);
	}
	...
	switch (argc) {
		case 1:
			...
			break;
		case 2:
			snprintf(result_string, sizeof(result_string)-1, "SELECT id FROM host WHERE (disabled='' and poller_id = %s) ORDER BY id", poller_id_ptr);
			result = db_query(&mysql, result_string);

			break;
		case 4:
			snprintf(result_string, sizeof(result_string)-1, "SELECT id FROM host WHERE (disabled='' and (id >= %s and id <= %s) and poller_id = %s) ORDER BY id\0", first_host_ptr, last_host_ptr, poller_id_ptr);
			result = db_query(&mysql, result_string);
	...
	if (set.verbose == POLLER_VERBOSITY_DEBUG) {
		snprintf(logmessage, LOGSIZE-1, "DEBUG: Initial Value of Active Threads is %i", active_threads);
		cacti_log(logmessage, SEV_DEBUG, 0);
	}
	...
	while ((device_counter < num_rows) && (canexit == 0)) {
			...
			while ((active_threads < set.threads) && (device_counter < num_rows)) {
				...
				switch (thread_status) {
					case 0:
						...
						if (set.verbose == POLLER_VERBOSITY_DEBUG) {
							snprintf(logmessage, LOGSIZE-1, "DEBUG: The Value of Active Threads is %i", active_threads);
							cacti_log(logmessage, SEV_DEBUG, 0);
						}
				...
				if ((current_time - begin_time) > poller_interval) {
					cacti_log("ERROR: Cactid Timed Out While Processing Hosts Internal", SEV_ERROR, 0);
					canexit = 1;
					break;
				}
			}
		...
		if ((current_time - begin_time) > poller_interval) {
			cacti_log("ERROR: Cactid Timed Out While Processing Hosts External", SEV_ERROR, 0);
			canexit = 1;
			break;
		}
	}
	...
	while (canexit == 0) {
		...
		if ((current_time - begin_time) > poller_interval) {
			cacti_log("ERROR: Cactid Timed Out While Processing Host Shutdown", SEV_ERROR, 0);
			canexit = 1;
			break;
		}
	}
	...
	snprintf(sql_string, sizeof(sql_string)-1, "insert into poller_time (poller_id, start_time, end_time) values (%i, '%s', NOW())",set.poller_id,start_datetime);
	...
	/* shutdown SNMP */
	if (set.verbose == POLLER_VERBOSITY_DEBUG) {
		snprintf(logmessage, LOGSIZE-1, "CACTID: Shutting down Net-SNMP API", VERSION);
		cacti_log(logmessage, SEV_INFO, 0);
	}
	snmp_cactid_close();
	...
	if (set.verbose == POLLER_VERBOSITY_DEBUG) {
		cacti_log("DEBUG: MYSQL Free & Close Completed", SEV_DEBUG, 0);
	}
	...
	if ((set.verbose >= POLLER_VERBOSITY_MEDIUM) && (argc != 1)) {
		snprintf(logmessage, LOGSIZE-1, "Execution Time: %.4f s, Threads: %i, Hosts: %i", (end_time - begin_time), set.threads, num_rows);
