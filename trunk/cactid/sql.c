...
int db_insert(MYSQL *mysql, char *query) {
	...
	if (set.verbose == POLLER_VERBOSITY_DEBUG) {
		snprintf(logmessage, LOGSIZE-1, "DEBUG: SQLCMD: %s", query);
		cacti_log(logmessage, SEV_DEBUG, 0);
	}

	thread_mutex_lock(LOCK_MYSQL);
	if (mysql_query(mysql, query)) {
		snprintf(logmessage, LOGSIZE-1, "ERROR: Problem with MySQL: %s", mysql_error(mysql));
...
int db_connect(char *database, MYSQL *mysql) {
	...
	if ((hostname = strdup(set.dbhost)) == NULL) {
		cacti_log("ERROR: malloc(): strdup() failed", SEV_DEBUG, 0);
		return (FALSE);
	}
	...
	if (set.verbose == POLLER_VERBOSITY_DEBUG) {
		snprintf(logmessage, LOGSIZE-1, "MYSQL: Connecting to MySQL database '%s' on '%s'...", database, set.dbhost);
		cacti_log(logmessage, SEV_DEBUG, 0);
	}
