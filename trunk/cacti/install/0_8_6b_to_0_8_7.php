...
function upgrade_to_0_8_7() {
	...
	/* New Logging table */
	db_install_execute("0.8.7", "CREATE TABLE `syslog` (`id` bigint(20) unsigned NOT NULL auto_increment,`logdate` datetime NOT NULL default '0000-00-00 00:00:00',`facility` enum('POLLER','CMDPHP','CACTID','SCPTSVR','AUTH','WEBUI','EXPORT','SMTP','UNKNOWN') NOT NULL default 'POLLER',`severity` enum('EMERGENCY','ALERT','CRITICAL','ERROR','WARNING','NOTICE','INFO','DEBUG') NOT NULL default 'EMERGENCY',`poller_id` smallint(5) unsigned NOT NULL default '0',`host_id` mediumint(8) unsigned NOT NULL default '0',`user_id` mediumint(8) unsigned NOT NULL default '0',`username` varchar(50) NOT NULL default 'system',`source` varchar(50) NOT NULL default 'localhost',`message` blob NOT NULL default '',PRIMARY KEY  (`id`),KEY `facility` (`facility`),KEY `severity` (`severity`),KEY `host_id` (`host_id`),KEY `poller_id` (`poller_id`),KEY `user_id` (`user_id`),KEY `username` (`username`),KEY `logdate` (`logdate`)) TYPE=MyISAM;");
}
?>