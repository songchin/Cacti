...
if (read_config_option("auth_method") == "2") {
	...
	log_save(_("LOGIN: Web Basic Authenication enabled, getting username from webserver"), SEV_DEBUG, FACIL_AUTH);
	$action = "login";
	if (isset($_SERVER["PHP_AUTH_USER"])) {
		$username = $_SERVER["PHP_AUTH_USER"];
		log_save(sprintf(_("LOGIN: Username set to '%s'"), $username), SEV_DEBUG, FACIL_AUTH);
	} else {
		/* No user - Bad juju! */
		$username = "";
		...
		log_save(_("No username passed with Web Basic Authentication enabled."), SEV_ERROR, FACIL_AUTH);
		exit;
	}
}else{
	if ($action == "login") {
		/* LDAP and Builtin get username from Form */
		log_save(_("LOGIN: Builtin or LDAP Authenication, getting username from form post"), SEV_DEBUG, FACIL_AUTH);
...
if ($action == 'login') {
	...
	case "0":
		log_save(_("LOGIN: No Authenication enabled"), SEV_DEBUG, FACIL_AUTH);
		/* No auth, no action, also shouldn't get here */
		exit;
		break;
	case "2":
		/* Web Basic Auth */
		log_save(_("LOGIN: Web Basic Authenication enabled"), SEV_DEBUG, FACIL_AUTH);
		$copy_user = true;
		$user_auth = true;
		$realm = 0;
	...
	case "3":
		/* LDAP Auth */
		log_save(_("LOGIN: LDAP Authentication enabled"), SEV_DEBUG, FACIL_AUTH);
 		if (($_POST["realm"] == "ldap") && (strlen($_POST["login_password"]) > 0)) {
			log_save(_("LOGIN: LDAP realm selected"), SEV_DEBUG, FACIL_AUTH);

			/* get user DN */
			log_save(_("LOGIN: LDAP Generating DN"), SEV_DEBUG, FACIL_AUTH);
			...
			if ($ldap_dn_search_response["error_num"] == "0") {
				...
				log_save(_("LOGIN: LDAP DN: ") . $ldap_dn, SEV_DEBUG, FACIL_AUTH);
			}else{
				/* Error searching */
				log_save(_("LOGIN: LDAP Error: ") . $ldap_dn_search_response["error_text"], SEV_ERROR, FACIL_AUTH);
				$ldap_error = true;
			...
			if (!$ldap_error) {
				/* auth user with LDAP */
				log_save(_("LOGIN: LDAP Authenication processing"), SEV_DEBUG, FACIL_AUTH);
				...
				if ($ldap_auth_response["error_num"] == "0") {
					...
					$copy_user = true;
					/* Locate user in database */
					log_save(sprintf(_("LOGIN: LDAP User '%s' Authenticated"),$username), SEV_DEBUG, FACIL_AUTH);
					...
					/* error */
					log_save(_("LOGIN: LDAP Error: ") . $ldap_auth_response["error_text"], SEV_ERROR, FACIL_AUTH);
					$ldap_error = true;
	...
	if (sizeof($user) > 0) {
		...
		if ($user_enabled == "0") {
			...
			log_save(_("LOGIN: URL: ") . $url_location, SEV_DEBUG, FACIL_AUTH);
		...
		if ((!$guest_user) && ($user_auth)) {
			...
			log_save(_("LOGIN: Access Denied, No guest enabled or template user to copy"), SEV_CRITICAL, FACIL_AUTH);
			exit;
		}else{
			/* BAD username/password builtin and LDAP */
			log_save(sprintf(_("LOGIN: Invalid username '%s' and password"), $username), SEV_WARNING, 0, 0, 0, false, FACIL_AUTH);
		}
	}
}
