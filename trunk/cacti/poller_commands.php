...
if (sizeof($poller_commands) > 0) {
	foreach ($poller_commands as $command) {
		...
		case POLLER_COMMAND_REINDEX:
			...
			if ($first_host) {
				api_syslog_cacti_log(_("Host[$host_id] WARNING: Recache Event Detected for Host"), SEV_WARNING, $poller_id, 0, 0, true, FACIL_POLLER);
			}
...
if ($recached_hosts > 0) {
	api_syslog_cacti_log($recache_stats, SEV_NOTICE, $poller_id, 0, 0, true, FACIL_POLLER);
}
