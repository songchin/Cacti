...
if (sizeof($poller_commands) > 0) {
	foreach ($poller_commands as $command) {
		...
		case POLLER_COMMAND_REINDEX:
			...
			if ($first_host) {
				api_log_log(_("Host[$host_id] WARNING: Recache Event Detected for Host"), SEV_WARNING, FACIL_POLLER, "", $poller_id, 0, 0, true);
			}

			api_log_log(_("Host[$host_id] RECACHE: Re-cache for Host, data query #$data_query_id"), SEV_NOTICE, FACIL_POLLER, "", $poller_id, 0, 0, true);

			api_data_query_execute($host_id, $data_query_id);

			api_log_log(_("Host[$host_id] RECACHE: Re-cache successful."), SEV_NOTICE, FACIL_POLLER, "", $poller_id, 0, 0, true);

			break;
		case POLLER_COMMAND_RRDPURGE:
			api_log_log(sprintf("_(Host[$i] PURGE: Unused RRDfile removed from system '%s')", $host_id, $command),SEV_NOTICE,  FACIL_POLLER, "", $poller_id, 0, 0, 0, true);
		...
		if (($current-$start) > MAX_RECACHE_RUNTIME) {
			api_log_log(sprintf(_("Poller Command processing timed out after processing '%s'"), $command, SEV_ERROR, FACIL_POLLER, "", $poller_id, 0, 0, true);
			break;
		}
	}
...
if ($recached_hosts > 0) {
	api_log_log($recache_stats, SEV_NOTICE, FACIL_POLLER, "", $poller_id, 0, 0, true);
}
