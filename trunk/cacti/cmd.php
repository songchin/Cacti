...
if ( $_SERVER["argc"] == 1 ) {
	...
	if ($_SERVER["argc"] == "4") {
		...
		foreach($parms as $parameter) {
   	   switch (substr($parameter,0,2)) {
				...
				case "-p":
					...
					break;
				default:
					cacti_log("Invalid Calling Parameter in CMD.PHP", SEV_ERROR, $poller_id, 0, 0, true, FACIL_CMDPHP);
			}
		}
		...
		if ($first_host <= $last_host) {
			...
			cacti_log("Invalid Arguments.  CMD.PHP calling parameters invalid.", SEV_ERROR, $poller_id, 0, 0, $print_data_to_stdout, FACIL_CMDPHP);
		}
	}else{
		print "ERROR: Invalid Arguments.  The first argument must be less than or equal to the first.\n";
		print "USAGE: CMD.PHP [-f=first_host -l=last_host -p=poller_id]\n";
...
if ($config["cacti_server_os"] == "win32") {
	...
	if ($guess == strtoupper($guess)) {
		cacti_log("The PHP Script: CMD.PHP Must be started using the full path to the file and in lower case.  This is a PHP Bug!!!", SEV_ERROR, $poller_id, 0, 0, false, FACIL_CMDPHP);
		exit(-1);
	}
}
...
if ((sizeof($polling_items) > 0) && (read_config_option("poller_enabled") == "on")) {
	...
	if (function_exists("proc_open")) {
		...
		if (substr_count($output, "Started") != 0) {
			if (read_config_option("log_verbosity") >= POLLER_VERBOSITY_HIGH) {
				cacti_log("PHP Script Server Started Properly", SEV_INFO, $poller_id, 0, 0, $print_data_to_stdout, FACIL_CMDPHP);
			}
		}
		...
		if (read_config_option("log_verbosity") == POLLER_VERBOSITY_DEBUG) {
			cacti_log("PHP version 4.3 or above is recommended for performance considerations.", SEV_NOTICE, $poller_id, 0, 0, $print_data_to_stdout, FACIL_CMDPHP);
		}
	}
		...
		if (($new_host) && (!empty($host_id))) {
			...
			if ((($ping_availability == AVAIL_SNMP) && ($item["snmp_community"] == "")) || ($ping_availability == AVAIL_NONE)) {
				...
				if (read_config_option("log_verbosity") >= POLLER_VERBOSITY_MEDIUM) {
					cacti_log("Availability Disabled for Host.", SEV_NOTICE, $poller_id, $host_id, 0, $print_data_to_stdout, FACIL_CMDPHP);
				}
			}else{
			...
			if (!$host_down) {
				...
				if ((sizeof($reindex) > 0) && (!$host_down)) {
					if (read_config_option("log_verbosity") == POLLER_VERBOSITY_DEBUG) {
						cacti_log("RECACHE: Processing " . sizeof($reindex) . " items in the auto reindex cache for '" . $item["hostname"] . "'.", SEV_DEBUG, $poller_id, $host_id, 0, $print_data_to_stdout, FACIL_CMDPHP);
					}
					...
					foreach ($reindex as $index_item) {
						...
						if (($index_item["op"] == "=") && ($index_item["assert_value"] != trim($output))) {
							cacti_log("ASSERT: '" . $index_item["assert_value"] . "=" . trim($output) . "' failed. Recaching host '" . $item["hostname"] . "', data query #" . $index_item["data_query_id"], SEV_DEBUG, $poller_id, $host_id, 0, $print_data_to_stdout, FACIL_CMDPHP);
		...
		if (!$host_down) {
			...
			case POLLER_ACTION_SNMP: /* snmp */
				...
				if (!validate_result($output)) {
					...
					cacti_log("Result from SNMP not valid.  Partial Result: " . substr($output, 0, $strout), SEV_NOTICE, $poller_id, $host_id, 0, $print_data_to_stdout, FACIL_CMDPHP);
					$output = "U";
				}

				if (read_config_option("log_verbosity") >= POLLER_VERBOSITY_MEDIUM) {
					cacti_log("SNMP: v" . $item["snmp_version"] . ": " . $item["hostname"] . ", dsname: " . $item["rrd_name"] . ", oid: " . $item["arg1"] . ", output: $output", SEV_INFO, $poller_id, $host_id, 0, $print_data_to_stdout, FACIL_CMDPHP);
				}
			...
			case POLLER_ACTION_SCRIPT: /* script (popen) */
				...
				if (!validate_result($output)) {
					...
					cacti_log("Result from CMD not valid.  Partial Result: " . substr($output, 0, $strout), SEV_NOTICE, $poller_id, $host_id, 0, $print_data_to_stdout, FACIL_CMDPHP);
					$output = "U";
				}

				if (read_config_option("log_verbosity") >= POLLER_VERBOSITY_MEDIUM) {
					cacti_log("CMD: " . $item["arg1"] . ", output: $output", SEV_INFO, $poller_id, $host_id, 0, $print_data_to_stdout, FACIL_CMDPHP);
				}
			...
			case POLLER_ACTION_SCRIPT_PHP: /* script (php script server) */
				if ($using_proc_function == true) {
					...
					if (!validate_result($output)) {
						...
						cacti_log("Result from SERVER not valid.  Partial Result: " . substr($output, 0, $strout), SEV_NOTICE, $poller_id, $host_id, 0, $print_data_to_stdout, FACIL_CMDPHP);
						$output = "U";
					}

					if (read_config_option("log_verbosity") >= POLLER_VERBOSITY_MEDIUM) {
						cacti_log("SERVER: " . $item["arg1"] . ", output: $output", SEV_INFO, $poller_id, $host_id, 0, $print_data_to_stdout, FACIL_CMDPHP);
					}
				}else{
					if (read_config_option("log_verbosity") >= POLLER_VERBOSITY_MEDIUM) {
						cacti_log("*SKIPPING* SERVER: " . $item["arg1"] . " (PHP < 4.3)", SEV_NOTICE, $poller_id, $host_id, 0, $print_data_to_stdout, FACIL_CMDPHP);
					}
	...
	if (($print_data_to_stdout) || (read_config_option("log_verbosity") >= POLLER_VERBOSITY_MEDIUM)) {
		...
		cacti_log(sprintf("Run Time: %01.4f s, Theads: N/A, Hosts: %s", round($end-$start,4), $host_count), SEV_NOTICE, $poller_id, 0, 0, $print_data_to_stdout, FACIL_CMDPHP);
	}

}else{
	cacti_log("Either there are no items in the cache or polling is disabled", SEV_ERROR, $poller_id, 0, 0, $print_data_to_stdout, FACIL_CMDPHP);
}
