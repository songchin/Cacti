	...
	if (isset($xml_array["fields"])) {
		...
		foreach ($xml_array["fields"] as $field_array) {
			if ((isset($field_array["name"])) && (isset($field_array["t_value"])) && (isset($field_array["value"]))) {
				...
				$save_fields{$field_array["name"]} = array("t_value" => $field_array["t_value"], "value" => $field_array["value"]);
			}
		}
...
function package_graph_template_import(&$xml_array, $package_id, $object_hash) {
	require_once(CACTI_BASE_PATH . "/lib/graph/package_info.php");
	require_once(CACTI_BASE_PATH . "/lib/graph_template/package_info.php");
	require_once(CACTI_BASE_PATH . "/lib/graph_template/package_update.php");
	...
	if ((isset($xml_array["template"])) && (isset($xml_array["graph"]))) {
		/* save the graph template to the database */
		$graph_template_id = api_graph_template_save(0, $save_fields);
	}
	...
	/* obtain a list of all graph item specific fields */
	$graph_fields = api_graph_template_item_form_list();

	if (isset($xml_array["items"])) {
		if (sizeof($xml_array["items"]) > 0) {
			foreach ($xml_array["items"] as $object_hash => $graph_item) {
