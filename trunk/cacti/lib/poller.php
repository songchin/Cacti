...
function exec_background($filename, $args = "", $poller_id = 1) {
	if (read_config_option("log_verbosity") == POLLER_VERBOSITY_DEBUG) {
		log_save(sprintf(_("About to Spawn a Remote Process [CMD: %s, ARGS: %s]"),$filename, $args), SEV_DEBUG, FACIL_POLLER, "", $poller_id);
	}
...
function update_poller_status($status, $poller_id, $pollers, $poller_time) {
	...
	if ($issue_log_message) {
		if ($pollers[$poller_id]["status"] == HOST_DOWN) {
			log_save(_("POLLER EVENT: Poller is DOWN Message: ") . $pollers[$poller_id]["status_last_error"], SEV_CRITICAL, FACIL_POLLER, "", $poller_id, $host_id, $print_data_to_stdout);
		} else {
			log_save(_("POLLER EVENT: Poller Returned from DOWN State"), SEV_NOTICE, FACIL_POLLER, "", $poller_id, $host_id, $print_data_to_stdout);
		}
	}
