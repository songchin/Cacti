...
function data() {
	...
	if (sizeof($data_inputs) > 0) {
		foreach ($data_inputs as $data_input) {
			...
			form_selectable_cell("<a class='linkEditMain' href='" . htmlspecialchars("data_input.php?action=edit&id=" . $data_input["id"]) . "'>" . (strlen($_REQUEST["filter"]) ? eregi_replace("(" . preg_quote($_REQUEST["filter"]) . ")", "<span class=\"filter\">\\1</span>", $data_input["name"]) : $data_input["name"]) . "</a>", $data_input["id"]);
