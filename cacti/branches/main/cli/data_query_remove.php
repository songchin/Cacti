...
if (sizeof($parms)) {
	...
	if (isset($dq["snmp_query_id"])) {
		foreach ($devices as $device) {
			...
			echo __("Data Query (%s) removed from Device (%s: %s)", $dq["snmp_query_id"], $device["id"], $device["devicename"]) . "\n";
		}
	}
}else{
	display_help($me);
...
function display_help($me) {
	echo __("Remove Data Query Script 1.0") . ", " . __("Copyright 2004-2010 - The Cacti Group") . "\n";
