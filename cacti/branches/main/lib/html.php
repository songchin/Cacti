...
function html_header_sort($header_items, $sort_column, $sort_direction, $last_item_colspan = 1) {
	...
	foreach($header_items as $db_column => $display_array) {
		...
		if (($db_column == "") || (substr_count($db_column, "nosort"))) {
			...
			print "\t\t\t<th style='width:$width;' id='rand_$rand_id'" . ((($rand_id+1) == count($header_items)) ? "colspan='$last_item_colspan' " : "") . " onMousemove='doColResize(this,event)' onMouseover='doColResize(this,event)' onMouseout='doneColResize()' class='textSubHeaderDark'>" . $display_text . "</th>\n";

			$rand_id++;
		}else{
			$width = html_get_column_width($pathname, $db_column);

			print "\t\t\t<th style='width: $width;' id='$db_column'" . ((($rand_id+1) == count($header_items)) ? "colspan='$last_item_colspan' " : "") . " onMousemove='doColResize(this,event)' onMouseover='doColResize(this,event)' onMouseout='doneColResize()' class='textSubHeaderDark'>";
