...
function rra() {
	...
	if (sizeof($rra_list) > 0) {
		foreach ($rra_list as $rra) {
			...
			form_selectable_cell("<a class='linkEditMain' href='" . htmlspecialchars("rra.php?action=edit&id=" . $rra["id"]) . "'>" . (strlen($_REQUEST["filter"]) ? eregi_replace("(" . preg_quote($_REQUEST["filter"]) . ")", "<span class=\"filter\">\\1</span>", $rra["name"]) : $rra["name"]) . "</a>", $rra["id"]);
