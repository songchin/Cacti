...
function get_host_array() {
	if (read_config_option("auth_method") != 0) {
		...
		$host_list = db_fetch_assoc("select id,".sql_function_concat("description","' ('","hostname","')'")." as name from host order by description,hostname");
	}
...
function get_associated_rras($local_graph_id) {
	...
	return db_fetch_assoc("select
		...
		from (graph_templates_item 
			cross join data_template_data_rra 
			cross join data_template_rrd
			cross join data_template_data
			cross join rra)
		...
		group by rra.id,rra.steps,rra.rows,rra.name,rra.timespan,data_template_data.rrd_step
