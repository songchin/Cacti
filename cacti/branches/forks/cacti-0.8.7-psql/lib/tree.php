...
function get_next_tree_id($order_key, $table, $field, $where) {
	...
	$order_key = db_fetch_cell("SELECT $field FROM $table WHERE $where AND $field LIKE '$parent_root%' ORDER BY $field DESC");
...
function move_branch($dir, $order_key, $table, $field, $where) {
	...
	if (sizeof($displaced_row) > 0) {
		...
		db_execute("UPDATE $table SET $field = ".sql_function_concat("'" . str_pad('', ($tier * CHARS_PER_TIER), 'Z') . "'",sql_function_substr()."($field," . (($tier * CHARS_PER_TIER) + 1).")")." WHERE $field LIKE '$new_root%'$where");
		db_execute("UPDATE $table SET $field = ".sql_function_concat("'$new_root'",sql_function_substr()."($field," . (($tier * CHARS_PER_TIER) + 1) . ")")." WHERE $field LIKE '$old_root%' $where");
		db_execute("UPDATE $table SET $field = ".sql_function_concat("'$old_root'",sql_function_substr()."($field," . (($tier * CHARS_PER_TIER) + 1) . ")")." WHERE $field LIKE '".str_pad('', ($tier * CHARS_PER_TIER), 'Z') . "%' $where");
	}
...
function sort_tree($sort_type, $item_id, $sort_style) {
	...
	$heirarchy = db_fetch_assoc("select
		...
		".sql_function_concat("description","' ('","hostname","')'")." as hostname
	...
	while (list($_tier_key, $tier_array) = each($leaf_sort_array)) {
		while (list($_search_key, $search_array) = each($tier_array)) {
			...
			db_execute("update graph_tree_items set order_key = ".sql_function_concat("'x'","order_key")." where order_key like '$_search_key%%' " . (($sort_type == SORT_TYPE_TREE_ITEM) ? "and id != $item_id" : "and order_key != '$_search_key" . str_repeat('0', (MAX_TREE_DEPTH * CHARS_PER_TIER) - strlen($_search_key)) . "'") . " and graph_tree_id = '$tree_id'");
			...
			while (list($leaf_order_key, $leaf_title) = each($search_array)) {
				...
				db_execute("update graph_tree_items set order_key = ".sql_function_replace()."(order_key, 'x$old_base_tier', '$new_base_tier') where order_key like 'x$old_base_tier%%' " . (($sort_type == SORT_TYPE_TREE_ITEM) ? "and id != $item_id" : "") . " and graph_tree_id = '$tree_id'");
