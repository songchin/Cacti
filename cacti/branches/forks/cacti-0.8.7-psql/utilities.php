...
//	print $user_log_sql;
	...
	$user_log = db_fetch_assoc($user_log_sql, read_config_option("num_rows_data_source"), (read_config_option("num_rows_data_source")*($_REQUEST["page"]-1)));
...
function utilities_view_snmp_cache() {
	...
	if ($_REQUEST["filter"] <> "") {
		$sql_where .= " AND (host.description LIKE '%%" . $_REQUEST["filter"] . "%%'
			...
			OR host_snmp_cache.objid LIKE '%%" . $_REQUEST["filter"] . "%%')";
	}
...
//	print $snmp_cache_sql;
	...
	$snmp_cache = db_fetch_assoc($snmp_cache_sql, read_config_option("num_rows_data_source"),(read_config_option("num_rows_data_source")*($_REQUEST["page"]-1)));
	...
	foreach ($snmp_cache as $item) {
		...
		?>
		<td>
			OID: <?php print eregi_replace("(" . preg_quote($_REQUEST["filter"]) . ")", "<span style='background-color: #F8D93D;'>\\1</span>", $item["objid"]);?>
		</td>
		</tr>
		<?php
	}
...
//	print $poller_sql;
	...
	$poller_cache = db_fetch_assoc($poller_sql, read_config_option("num_rows_data_source"),(read_config_option("num_rows_data_source")*($_REQUEST["page"]-1)));
