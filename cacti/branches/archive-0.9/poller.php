...
if ($poller_id == 1) {
	...
	while (1) {
		...
		if (($start + MAX_POLLER_RUNTIME) < time()) {
			log_save("Maximum runtime of " . MAX_POLLER_RUNTIME . " seconds exceeded for Poller_ID " . $poller_id . " - Exiting.", SEV_ERROR, FACIL_POLLER, "", $poller_id, 0, true);
...
if (read_config_option("poller_enabled") == "on") {
	...
	if (($poller_type == "2") && (!file_exists(read_config_option("path_cactid")))) {
		log_save(sprintf(_("ERROR: The path: %s is invalid.  Can not continue"),read_config_option("path_cactid")), SEV_CRITICAL, FACIL_POLLER, "", $poller_id, 0, true);
		exit;
	}
	...
	log_save(sprintf(_("SystemTime:") . "%01.4f " .
		_("TotalPollers:") . "%s " .
		_("Method:") . "%s " .
		_("Processes:") . "%s " .
		_("Threads:") . "%s " .
	...
	if ($poller_id == 1) {
		log_save(_("Either there are no pollers enabled, no items in your poller cache or polling is disabled. Make sure you have at least one data source created, your poller is active. If both are true, go to 'Utilities', and select 'Clear Poller Cache'."), SEV_CRITICAL, FACIL_POLLER, "", $poller_id, 0, true);
	} else {
		db_execute("update poller set run_state = 'Complete' where id=" . $poller_id);
		log_save(_("Poller had not items to process."), SEV_CRITICAL, FACIL_POLLER, "", $poller_id, 0, true);
	}
}
